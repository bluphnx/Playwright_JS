...
test.beforeAll(async () => {
    ...
    // this is same as the the token we received from the login response token
    ...
    ordersID = orderResponeJson.orders[0];
    console.log('orderResponeJson.orders[0]: ' + ordersID);
    productOrderId = orderResponeJson.productOrderId[0];
    console.log('orderResponeJson.productOrderId[0]: ' + productOrderId);
    orderResponeMessage = orderResponeJson.message;
...
test("E2E - webapi - place order in Client website", async ({ browser }) => {
    ...
    */
    ...
    // validate the order summary page with code
    ...
    for (let index = 0; index < orderCount; index++) {
        ...
        if (await page.locator("tbody tr th").nth(index).textContent() === ordersID)   // api
        {
    ...
    await expect(page.locator("div.col-text")).toHaveText(ordersID);    // API
